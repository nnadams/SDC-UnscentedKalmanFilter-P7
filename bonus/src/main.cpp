    ...
    if (length && length > 2 && data[0] == '4' && data[1] == '2') {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          double actual_distence =
            sqrt((target_y - hunter_y) * (target_y - hunter_y) +
                 (target_x - hunter_x) * (target_x - hunter_x));
          cout << "Distance from car: " << actual_distence << endl;

          double lookahead_time;
          if (actual_distence > 3)
            lookahead_time = actual_distence / 8.5;
          else
            lookahead_time = 0.30;

          VectorXd x_guess = ukf.FuturePrediction(lookahead_time);
